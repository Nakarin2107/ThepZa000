...
async function signupUser() {
    ...
    storedUsers.push({ displayName, username, password: hashedPassword });
...
function showAllUsers() {
    ...
    // สร้างรายการชื่อผู้ใช้ทั้งหมด
    const userList = storedUsers.map(user => user.username).join('<br>');

    // แสดงผลรายการชื่อผู้ใช้
    Swal.fire({
        icon: 'info',
        title: 'รายการชื่อผู้ใช้ที่เคยสมัครไว้',
        html: userList, // ใช้ 'html' เพื่อแสดงชื่อผู้ใช้หลายบรรทัด
        confirmButtonText: 'ตกลง'
    });
}
